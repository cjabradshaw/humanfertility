...
##
## BRADSHAW, CJA, C PERRY, M JUDGE, CM SARASWATI, J HEYWORTH, PN LE SOUËF. In review. Lower infant mortality,
## household size, and access to contraception reduce fertility in low- and middle-income nations. PLoS One
##
## Corey J. A. Bradshaw
## Flinders University
## August 2022
...
final.dat.imputed <- data.frame(dat$WBregion, dat$tfr10t20, homevis.imp.sc, Feduc.imp.sc, CathMusl.imp.sc, infmort.imp.sc, anyContr.imp.sc, quality.imp.sc, housesiz.imp.sc)
colnames(final.dat.imputed) <- c("reg", "fert", "homevis","Feduc","CathMusl","infmort","anyContr", "quality", "housesiz")
final.datNA <- data.frame(dat$WBregion, dat$tfr10t20, homevst.sc, educyrsmn.sc, cathmusl.sc, imr.sc, anycontr.sc, quality.sc, meanmem.sc)
colnames(final.datNA) <- c("reg", "fert", "homevis","Feduc","CathMusl","infmort","anyContr", "quality", "housesiz")

## global model
## homevis, Feduc, CathMusl, infmort, anyContr, quality, housesiz

final.dat <- final.dat.imputed
dim(final.dat)

cormat.final <- cor(na.omit(final.dat[,-c(1,2)]), method="kendall")
cormat.final
...
## boosted regression tree
# transformed values
#final.dat <- final.datNA
brt.final <- gbm.step(final.dat, gbm.x = attr(final.dat, "names")[c(3:9)], gbm.y = attr(final.dat, "names")[2], family="gaussian", tolerance = 0.001, learning.rate = 0.0001, bag.fraction=0.75, tree.complexity = 2, tolerance.method = "auto", plot.main=T, plot.folds=F, max.trees=100000)
summary(brt.final)
gbm.plot(brt.final)
...
val.arr <- pred.arr <- array(data = 0, dim = c(eq.sp.points, 7, iter), dimnames=list(paste("x",1:eq.sp.points,sep=""), attr(final.dat, "names")[c(3:9)], paste("b",1:iter,sep="")))
...
for (b in 1:iter) {  # start b loop
  ...
  brt.fit <- gbm.step(final.boot, gbm.x = attr(final.boot, "names")[c(3:9)], gbm.y = attr(final.boot, "names")[2], family="gaussian", tolerance = 0.0001, learning.rate = 0.001, bag.fraction=0.8, tree.complexity = 2, tolerance.method = "auto", plot.main=T, plot.folds=F, max.trees=100000, silent=T)
  summ.fit <- summary(brt.fit)
  
  homevis.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][1])]
  Feduc.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][2])]
  CathMusl.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][3])]
  infmort.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][4])]
  anyContr.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][5])]
  quality.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][6])]
  housesiz.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(final.boot, "names")[c(3:9)][7])]
...
# raw values
final.dat.raw <- data.frame(dat$tfr10t20, dat$homevst, dat$educyrsmn, dat$ardacathmusl2020, dat$imr2020, dat$anycontr, dat$quality, dat$meanmem)
colnames(final.dat.raw) <- c("fert", "homevis","Feduc","CathMusl","infmort","anyContr","quality","housesiz")
brt.finalraw <- gbm.step(final.dat.raw, gbm.x = attr(final.dat.raw, "names")[c(2:8)], gbm.y = attr(final.dat.raw, "names")[1], family="gaussian", tolerance = 0.00001, learning.rate = 0.0001, bag.fraction=0.75, tree.complexity = 2, tolerance.method = "auto", plot.main=T, plot.folds=F, max.trees=100000)
...
infmort.scale <- as.numeric(infmort.imp.sc) * attr(infmort.imp.sc, 'scaled:scale')
...
infmort.scale.fit <- as.numeric(infmort.val) * attr(infmort.imp.sc, 'scaled:scale')
...
#write.table(final.dat.centroids,file="fert.gls.csv",sep=",", row.names = F, col.names = T)
